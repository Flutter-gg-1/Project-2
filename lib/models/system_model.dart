...
// for categories
  ...
  void addAdmin() {
    ...
    usermModelLis
        .add(UserModel(userName: userName, passWord: passowrd, isAdmin: true));
  }
  ...
  void appRun(SystemModel library) {
    ...
    while (true) {
      ...
      if (isNew == "t") {
        ...
        usermModelLis.add(
            UserModel(userName: userName, passWord: passowrd, isAdmin: false));
      } else if (isNew == "f") {
        print("\n######      plase give me your userName     ######\n");
        ...
        for (int i = 0; i < usermModelLis.length; i++) {
          if (usermModelLis[i].userName == userName &&
              usermModelLis[i].passWord == passowrd) {
